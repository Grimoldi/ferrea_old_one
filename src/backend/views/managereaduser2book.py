...
class ManageReadUser2Book(APIView):
    '''ManageReadUser2Book This class handles the creation or update of a READ relationship.

    This class creates or updates a READ relationship between a User node and a Book node.
    '''
    ...
    def post(self, request, format=None):
        '''post Method to create the READ relationship.
        ...
        if is_info_successfull:
            ...
            if is_books_count_successfull:
                ...
                if is_already_at_max:
                    ...
                    return self.dao.create_read(**create_info.data)
            else:
                return books_hold
        else:
            return create_info
    ...
    def put(self, request, format=None):
        '''put Method to update the READ relationship.

        This public method override the put method.
        This method gets the parameters from the put and call the method to update the relationship on db.
        Expected data in request.data are
        {
            username,
            barcode,
        ...
        First, it checks if the user is holding more books than permitted.
        If not, it updates the read relationship in order to set the "to" attribute.
        ...
        update_info = self._retrieve_params(request)
        is_info_successfull = update_info.status_code == 200
        if is_info_successfull:
            return self.dao.update_read(**update_info.data)
        else:
            return update_info
